...
Thus, the target set, when $d=2$, is $P =\{ v_1,v_7, v6\}$ which are picked from the 3 iterations. If we compare the cardinality of the target set when $d=1$ and $d=2$, we can see that d=2 has less cardinality than $d=1$ that means $d=2$ is better to use than $d=1$ because target set cardinality is minimized.

...
For an exhaustive way of getting the target set of a graph is to have the obtain the power set of the vertices which is takes $O(|V|^2)$. Then we check if each node can activate all of the nodes left in the graph, this process itself is non-deterministic. 

\begin{minipage}{0.45\textwidth}
	\begin{tikzpicture}[-,>=stealth',shorten >=1pt,auto,node distance=4cm,
	thick,main node/.style={circle,draw,font=\sffamily\small\bfseries}]
	
	\node[main node,label={$2$}] (A) {$A$};
	\node[main node,label=below:{$2$}] (C) [below of=A] {$C$};
	\node[main node,label={$2$}] (B) [right of=A] {$B$};
	\node[main node,label=below:{$2$}] (D) [right of=C] {$D$};
...
\begin{minipage}{0.45\textwidth}
	...
	\node[main node,label={$3$}] (A) {$A$};
	\node[main node,label=below:{$3$}] (C) [below of=A] {$C$};
	\node[main node,label={$3$}] (B) [right of=A] {$B$};
	\node[main node,label=below:{$3$}] (D) [right of=C] {$D$};
...
\begin{minipage}{0.45\textwidth}
	...
	\node[main node,label={$2$}] (A) {$A$};
	\node[main node,label={$2$}](E) [below right of=A] {$E$};
	\node[main node,label=below:{$2$}] (C) [below of=A] {$C$};
	\node[main node,label={$2$}] (B) [right of=A] {$B$};
	\node[main node,label=below:{$2$}] (D) [right of=C] {$D$};
...
\begin{minipage}{0.45\textwidth}
	...
	\node[main node,label={$3$}] (A) {$A$};
	\node[main node,label={$3$}](E) [below right of=A] {$E$};
	\node[main node,label=below:{$3$}] (C) [below of=A] {$C$};
	\node[main node,label={$3$}] (B) [right of=A] {$B$};
	\node[main node,label=below:{$3$}] (D) [right of=C] {$D$};
...
\end{tikzpicture}
\end{figure}
The value of the parameters for the algorithm are $\rho=0.60284$ and $d=3$, making the output a small 3-seeding target set. The ceiling value of $n_v^{(a)}$ which is equal to $\rho \cdot d(v)$ was used to avoid 0 $n_v^{(a)}$ at the start of the iteration. The table \ref{viradstable} contains the initial values needed in the algorithm. The iterations will show what are added into the target set $P$ denoted by the blue color node while the activated nodes are yellow nodes.
\begin{table}[ht]
\centering
...
In comparison of runtime, we do not compare the runtime between algorithms. We try to compare the normal versus random eligible vertex selection variations of the algorithms. 
...
We try the TSS algorithm to random graphs. We construct the random graphs by specifying the number of nodes we want $n$ and the probability that an edge occurs $p$. To get the exact number of nodes in the target set we attempt to formulate the problem into a 0-1 Linear Integer Programming problem from both \cite{}\cite{virads}. We apply the ILP and the TSS to relatively small networks compared to real-life social networks $n=30\ and\ n=50$ since solving ILP problems for huge networks would take days to weeks\cite{virads}. Using the add-in solver in Excel, we obtain these results:

...
\end{table}
 ...
 We first choose the vertex with minimum $dist$ which in this case is $v_{9}$, we remove $v_{9}$ from the graph and update the $dist$ of their neighbors. We set the $dist$ of the vertices with 0 or negative $dist$ values as $\infty$, meaning they are the maximum or not considered in the selection of current $v$.
 \begin{minipage}{0.45\textwidth}
...
\begin{figure}[h!]
  ...
  \node[main node,label={right:$v_1$}] (1) {$2$};
  \node[main node,label={above right:$v_2$}, below of=1] (2) {$1$};
  \node[main node,label={right:$v_3$}, below left of=2] (3) {$1$};
  \node[main node,label={below:$v_4$}, below of=2] (4) {$1$};
  \node[main node,label={right:$v_5$}, below right of=2] (5) {$3$};
  \node[main node,label={right:$v_6$},right of=2] (6) {$1$};
  \node[main node,label={right:$v_7$},above right of=6] (7) {$2$};

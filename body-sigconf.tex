...
\begin{figure}[h]
  ...
  \path[every node/.style={font=\sffamily\large}]
    (A) edge [bend left] node {$0.5$} (B)
    	edge node {$0.4$} (C)
    (B) edge node {$0.2$} (A)
    	edge [bend left] node {$0.3$} (C)
    	edge [bend left] node {$0.2$} (D)
    (C) edge [bend left] node {$0.3$} (A)
    	edge  node {$0.7$} (B)
    	edge node {$0.5$} (D)
    (D) edge [bend left] node {$0.3$} (C)
    	edge node {$0.4$} (B)
    ;
		\end{tikzpicture}
	\end{minipage}
...
Given a directed simple graph $G(V,E)$,we assign a probability $P_{u,v}$, this defines the probability of $y$ infecting $v$. Each node can try to influence its neighbor. When a node is influenced successfully, the node being influenced becomes active. If a node fails to influence a neighbor they cannot try to influence that node agian.\cite{Kempe}\cite{Shakarian:2012:LSN:2456719.2457081}
\begin{figure}[h]
	\begin{minipage}{0.45\textwidth}
		\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=4cm,
                    thick,main node/.style={circle,draw,font=\sffamily\small\bfseries}]
...
\end{tikzpicture}
  ...
  \node[main node] (A) {$A$};
  \node[main node,color={red}] (C) [below of=A] {$C$};
  \node[main node,color={gray}] (B) [right of=A] {$B$};
  \node[main node,color={red}] (D) [right of=C] {$D$};
...
\end{tikzpicture}
  ...
  \node[main node,label={0.4}, color={red}] (A) {$A$};
  \node[main node,label={0.8},color={red}] (C) [below of=A] {$C$};
  \node[main node,label={0.2}, color={gray}] (B) [right of=A] {$B$};
  \node[main node,label={0.3}, color={red}] (D) [right of=C] {$D$};
...
\end{figure}
\subsection{The threshold model used in study}

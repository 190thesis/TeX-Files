...
\begin{tikzpicture}[-,>=stealth',shorten >=1pt,auto,node distance=3cm,
  ...
  \path[every node/.style={font=\sffamily\large}]
    ...
    (3) edge [bend left,pos=0.2] node {$e_4$} (5)
        edge node [ bend right,pos=0.5] {$e_7$} (4)
        edge [bend right] node {$e_8$} (5)
    (4) edge node {$e_5$} (5)
	(5) 
    ;
\end{tikzpicture}
...
\item The vertex $v$ is \textit{isolated} if $v$ is not in $\phi(e)$ for all $e$ in $E$. In the figure \ref{graph1}, the vertex $v_6$ is isolated.
	\end{itemize}
...
\caption{An example of a null graph on 4 vertices}
	...
	\item A graph $G=(V,E)$ is a \textit{path graph} on $n\geq2$ vertices if,
		\begin{enumerate}
			\item $V=\{v_1,v_2,...,v_n\}$
			\item $E=\{\{v_1,v_2\},\{v_2,v_3\},...,\{v_{n-1},v_{n}\}\}$
		\end{enumerate}
  ...
  \path[every node/.style={font=\sffamily\large}]
    ...
    ;
\end{tikzpicture}
\caption{An example of a path graph with 4 vertices}
		\end{figure}
	\item A graph $G=(V,E)$ is a \textit{cycle graph or cycle} on $n\geq3$ vertices such that $n \in \mathbb{N}$ if,
	\begin{enumerate}
		\item $V=\{v_1,v_2,...,v_n\}$
...
\end{figure}	 
\end{itemize}
\subsection{Degrees, Regular Graph and Subgraphs}
...
\begin{itemize}
	...
	$$d(v)=|\{e\in E: v \in \phi(e), |\phi(e)|=2 \}|+2|\{e\in E: v \in \phi(e), |\phi(e)|=1 \}|$$
...
\end{equation}
\\ \\
 \item Hand Shake Theorem \\ The \textit{hand shake theorem} was inspired from the hand shaking scenario on a party. Each person in the party needs to shake hands with the other persons in the room. If all the persons in the party asked how many other persons they have shake hands with, the sum would be twice the number of total handshakes. In parallel to graphs, people in the party represents the vertex set of the graph, the shake hands made are the edge set of the graph and the amount of each person's handshake to other persons. The graph $G=(V,E)$ has a total degree of,
